...
const BillViewModal: React.FC<BillViewModalProps> = ({ bill, isOpen, onClose, isEstimateView = false }) => {
  ...
  const generatePdfHtml = (): string => {
    const logoHtml = company.showCompanyLogo 
      ? company.companyLogo 
        ? `<img src="${company.companyLogo}" alt="${company.companyName} Logo" style="max-width: 100px; max-height: 50px; object-fit: contain; margin-bottom: 8px;" />` // Reduced logo size
        : `<div style="width: 80px; height: 40px; border: 1px solid #ccc; display: flex; align-items: center; justify-content: center; margin-bottom: 8px; font-size: 10pt; color: #555;">Logo</div>`
      : '';
    
    const companyGstinDisplay = (!isEstimateView && bill.companyGstin) ? `<p style="font-size: 9pt; margin: 0 0 4px 0; color: #000000;">GSTIN: ${bill.companyGstin}</p>` : '';
    ...
    const itemsHtml = bill.items.map((item, index) => {
      ...
      return `
        <tr style="font-size: 9pt; page-break-inside: avoid;">
          <td style="border: 1px solid #333333; padding: 5px; text-align: center;">${index + 1}</td>
          <td style="border: 1px solid #333333; padding: 5px;">${item.name} ${valuableDetails ? `(${valuableDetails.name})` : ''}</td>
          ${showHsnInPdf ? `<td style="border: 1px solid #333333; padding: 5px; text-align: center;">${item.hsnCode || '-'}</td>` : ''}
          <td style="border: 1px solid #333333; padding: 5px; text-align: right;">${item.weightOrQuantity.toFixed(item.unit === 'carat' || item.unit === 'ct' ? 3 : 2)} ${item.unit}</td>
          <td style="border: 1px solid #333333; padding: 5px; text-align: right;">${currency}${effectiveRate.toFixed(2)}</td>
          ${bill.type === 'sales-bill' && bill.items.some(i => i.makingCharge && i.makingCharge > 0) ? `<td style="border: 1px solid #333333; padding: 5px; text-align: right;">${item.makingCharge && item.makingCharge > 0 ? (item.makingChargeType === 'percentage' ? `${item.makingCharge}%` : currency + item.makingCharge.toFixed(2)) : '-'}</td>` : ''}
          ${showItemGstCols ? `<td style="border: 1px solid #333333; padding: 5px; text-align: right;">${currency}${taxableAmount.toFixed(2)}</td>` : ''}
          ${showItemGstCols ? `
            <td style="border: 1px solid #333333; padding: 5px; text-align: right;">${currency}${itemCgst.toFixed(2)}<br/><span style="font-size:7pt;">(${settings.cgstRate}%)</span></td>
            <td style="border: 1px solid #333333; padding: 5px; text-align: right;">${currency}${itemSgst.toFixed(2)}<br/><span style="font-size:7pt;">(${settings.sgstRate}%)</span></td>
          ` : ''}
          <td style="border: 1px solid #333333; padding: 5px; text-align: right; font-weight: bold;">${currency}${lineTotal.toFixed(2)}</td>
        </tr>
      `;
    }).join('');
    ...
    let tableHeaders = `
      <th style="border: 1px solid #333333; padding: 6px; text-align: center; font-weight: bold; background-color: #e0e0e0; font-size: 9pt;">#</th>
      <th style="border: 1px solid #333333; padding: 6px; text-align: left; font-weight: bold; background-color: #e0e0e0; font-size: 9pt;">Item Description</th>
    `;
    if (showHsnColInPdfForHeader) tableHeaders += `<th style="border: 1px solid #333333; padding: 6px; text-align: center; font-weight: bold; background-color: #e0e0e0; font-size: 9pt;">HSN</th>`;
    tableHeaders += `<th style="border: 1px solid #333333; padding: 6px; text-align: right; font-weight: bold; background-color: #e0e0e0; font-size: 9pt;">Qty/Wt</th>`;
    tableHeaders += `<th style="border: 1px solid #333333; padding: 6px; text-align: right; font-weight: bold; background-color: #e0e0e0; font-size: 9pt;">Rate / ${(bill.items[0]?.unit || 'unit')}</th>`;
    if (showMakingChargeColumnInPdf) tableHeaders += `<th style="border: 1px solid #333333; padding: 6px; text-align: right; font-weight: bold; background-color: #e0e0e0; font-size: 9pt;">Making</th>`;
    if (showItemTaxableColInPdf) tableHeaders += `<th style="border: 1px solid #333333; padding: 6px; text-align: right; font-weight: bold; background-color: #e0e0e0; font-size: 9pt;">Taxable Amt</th>`;
    if (showItemGstColsInPdf) {
      tableHeaders += `<th style="border: 1px solid #333333; padding: 6px; text-align: right; font-weight: bold; background-color: #e0e0e0; font-size: 9pt;">CGST</th>`;
      tableHeaders += `<th style="border: 1px solid #333333; padding: 6px; text-align: right; font-weight: bold; background-color: #e0e0e0; font-size: 9pt;">SGST</th>`;
    }
    tableHeaders += `<th style="border: 1px solid #333333; padding: 6px; text-align: right; font-weight: bold; background-color: #e0e0e0; font-size: 9pt;">Line Total</th>`;
    ...
    return `
    <div id="bill-content-for-pdf" style="font-family: 'PT Sans', Arial, sans-serif; color: #000000; width: 100%; max-width: 794px; margin: 0 auto; padding: 15px; background-color: #ffffff; border: 1px solid #ccc; box-sizing: border-box;">
        
        <div class="bill-company-header" style="text-align: center; margin-bottom: 20px;">
          ${logoHtml}
          <h1 style="font-family: 'Playfair Display', serif; font-size: 22pt; margin: 0 0 5px 0; color: #000000;">${company.companyName}</h1>
          ${company.slogan ? `<p style="font-size: 11pt; margin: 0 0 5px 0; color: #000000;">${company.slogan}</p>` : ''}
          <p style="font-size: 9pt; margin: 0 0 3px 0; color: #000000;">${company.address}</p>
          <p style="font-size: 9pt; margin: 0 0 3px 0; color: #000000;">Phone: ${company.phoneNumber}</p>
          ${companyGstinDisplay}
        </div>

        <h2 class="bill-type-heading" style="font-family: 'Playfair Display', serif; font-size: 18pt; text-align: center; margin: 20px 0; font-weight: bold; text-transform: uppercase; border-top: 1.5px solid #000000; border-bottom: 1.5px solid #000000; padding: 6px 0;">${effectiveBillType}</h2>
        
        <table class="bill-meta-grid" style="width: 100%; margin-bottom: 20px; font-size: 9pt;">
          <tr>
            <td style="width: 50%; vertical-align: top; padding-right: 10px;">
              <h3 style="font-size: 11pt; font-weight: bold; margin-bottom: 6px;">Bill Details:</h3>
            ...
            <td style="width: 50%; text-align: right; vertical-align: top; padding-left: 10px;">
              <h3 style="font-size: 11pt; font-weight: bold; margin-bottom: 6px;">${customerLabel}</h3>
              <p style="margin: 4px 0; font-weight: bold;">${bill.customerName || 'N/A'}</p>
              ${bill.customerAddress ? `<p style="font-size: 9pt; margin: 4px 0;">${bill.customerAddress}</p>` : ''}
              ${bill.customerPhone ? `<p style="font-size: 9pt; margin: 4px 0;">Phone: ${bill.customerPhone}</p>` : ''}
            </td>
          </tr>
        </table>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; font-size: 9pt;">
          <thead>
            <tr style="font-size: 9pt;">${tableHeaders}</tr>
          </thead>
        ...
        <table class="bill-summary-grid" style="width: 100%; margin-top: 20px; font-size: 10pt;">
          <tr>
            <td style="width: 60%; vertical-align: top; white-space: pre-line;">
              ${bill.notes ? `<h4 style="font-weight: bold; margin-bottom: 5px; font-size: 10pt;">Notes:</h4><p style="font-size:9pt;">${bill.notes}</p>` : ''}
            </td>
            <td style="width: 40%; text-align: right; vertical-align: top; padding-left: 15px;">
              ...
              ${isEstimateView && bill.type === 'sales-bill' ? `<p style="font-size: 8pt; color: #333; margin: 5px 0;">(GST not applicable for estimates)</p>` : ''}
              <hr style="border: 0; border-top: 1px solid #555555; margin: 8px 0;" />
              <p style="font-size: 13pt; font-weight: bold; margin-top: 8px;">Total: <span style="font-weight: bold; font-size: 15pt;">${currency}${bill.totalAmount.toFixed(2)}</span></p>
            </td>
          </tr>
        </table>

        <div class="bill-footer" style="text-align: center; margin-top: 30px; padding-top: 15px; border-top: 1.5px solid #000000; font-size: 9pt;">
          <p>Thank you for your business!</p>
          <p style="margin-top: 6px;">--- ${company.companyName} ---</p>
        </div>
      </div>
    `;
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-h-[95vh] flex flex-col print-dialog-content text-base w-full max-w-screen-md"> 
