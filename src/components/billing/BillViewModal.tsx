...
const BillViewModal: React.FC<BillViewModalProps> = ({ bill, isOpen, onClose, isEstimateView = false }) => {
  ...
  const generatePdfHtml = (): string => {
    const logoHtml = company.showCompanyLogo 
      ? company.companyLogo 
        ...
        : `<div style="width: 100px; height: 50px; border: 1px solid #ccc; display: flex; align-items: center; justify-content: center; margin-bottom: 10px; font-size: 12pt; color: #555;">Logo</div>`
      : '';
    ...
    const itemsHtml = bill.items.map((item, index) => {
      ...
      return `
        <tr style="font-size: 10pt; page-break-inside: avoid;">
          <td style="border: 1px solid #333333; padding: 6px; text-align: center;">${index + 1}</td>
          <td style="border: 1px solid #333333; padding: 6px;">${item.name} ${valuableDetails ? `(${valuableDetails.name})` : ''}</td>
          ${showHsnInPdf ? `<td style="border: 1px solid #333333; padding: 6px; text-align: center;">${item.hsnCode || '-'}</td>` : ''}
          <td style="border: 1px solid #333333; padding: 6px; text-align: right;">${item.weightOrQuantity.toFixed(item.unit === 'carat' || item.unit === 'ct' ? 3 : 2)} ${item.unit}</td>
          <td style="border: 1px solid #333333; padding: 6px; text-align: right;">${currency}${effectiveRate.toFixed(2)}</td>
          ${bill.type === 'sales-bill' && bill.items.some(i => i.makingCharge && i.makingCharge > 0) ? `<td style="border: 1px solid #333333; padding: 6px; text-align: right;">${item.makingCharge && item.makingCharge > 0 ? (item.makingChargeType === 'percentage' ? `${item.makingCharge}%` : currency + item.makingCharge.toFixed(2)) : '-'}</td>` : ''}
          ${showItemGstCols ? `<td style="border: 1px solid #333333; padding: 6px; text-align: right;">${currency}${taxableAmount.toFixed(2)}</td>` : ''}
          ${showItemGstCols ? `
            <td style="border: 1px solid #333333; padding: 6px; text-align: right;">${currency}${itemCgst.toFixed(2)}<br/><span style="font-size:8pt;">(${settings.cgstRate}%)</span></td>
            <td style="border: 1px solid #333333; padding: 6px; text-align: right;">${currency}${itemSgst.toFixed(2)}<br/><span style="font-size:8pt;">(${settings.sgstRate}%)</span></td>
          ` : ''}
          <td style="border: 1px solid #333333; padding: 6px; text-align: right; font-weight: bold;">${currency}${lineTotal.toFixed(2)}</td>
        </tr>
      `;
    }).join('');
    ...
    let tableHeaders = `
      <th style="border: 1px solid #333333; padding: 7px; text-align: center; font-weight: bold; background-color: #e0e0e0;">#</th>
      <th style="border: 1px solid #333333; padding: 7px; text-align: left; font-weight: bold; background-color: #e0e0e0;">Item Description</th>
    `;
    if (showHsnColInPdfForHeader) tableHeaders += `<th style="border: 1px solid #333333; padding: 7px; text-align: center; font-weight: bold; background-color: #e0e0e0;">HSN</th>`;
    tableHeaders += `<th style="border: 1px solid #333333; padding: 7px; text-align: right; font-weight: bold; background-color: #e0e0e0;">Qty/Wt</th>`;
    tableHeaders += `<th style="border: 1px solid #333333; padding: 7px; text-align: right; font-weight: bold; background-color: #e0e0e0;">Rate / ${(bill.items[0]?.unit || 'unit')}</th>`;
    if (showMakingChargeColumnInPdf) tableHeaders += `<th style="border: 1px solid #333333; padding: 7px; text-align: right; font-weight: bold; background-color: #e0e0e0;">Making</th>`;
    if (showItemTaxableColInPdf) tableHeaders += `<th style="border: 1px solid #333333; padding: 7px; text-align: right; font-weight: bold; background-color: #e0e0e0;">Taxable Amt</th>`;
    if (showItemGstColsInPdf) {
      tableHeaders += `<th style="border: 1px solid #333333; padding: 7px; text-align: right; font-weight: bold; background-color: #e0e0e0;">CGST</th>`;
      tableHeaders += `<th style="border: 1px solid #333333; padding: 7px; text-align: right; font-weight: bold; background-color: #e0e0e0;">SGST</th>`;
    }
    tableHeaders += `<th style="border: 1px solid #333333; padding: 7px; text-align: right; font-weight: bold; background-color: #e0e0e0;">Line Total</th>`;
    ...
    return `
    <div id="bill-content-for-pdf" style="font-family: 'PT Sans', Arial, sans-serif; color: #000000; width: 100%; max-width: 794px; margin: 0 auto; padding: 20px; background-color: #ffffff; border: 1px solid #ccc; box-sizing: border-box;">
        
        <div class="bill-company-header" style="text-align: center; margin-bottom: 25px;">
          ${logoHtml}
          <h1 style="font-family: 'Playfair Display', serif; font-size: 26pt; margin: 0 0 6px 0; color: #000000;">${company.companyName}</h1>
          ${company.slogan ? `<p style="font-size: 12pt; margin: 0 0 6px 0; color: #000000;">${company.slogan}</p>` : ''}
          <p style="font-size: 10pt; margin: 0 0 4px 0; color: #000000;">${company.address}</p>
          <p style="font-size: 10pt; margin: 0; color: #000000;">Phone: ${company.phoneNumber}</p>
        </div>

        <h2 class="bill-type-heading" style="font-family: 'Playfair Display', serif; font-size: 20pt; text-align: center; margin: 25px 0; font-weight: bold; text-transform: uppercase; border-top: 2px solid #000000; border-bottom: 2px solid #000000; padding: 8px 0;">${effectiveBillType}</h2>
        
        <table class="bill-meta-grid" style="width: 100%; margin-bottom: 25px; font-size: 10pt;">
          <tr>
            <td style="width: 50%; vertical-align: top; padding-right: 15px;">
              <h3 style="font-size: 12pt; font-weight: bold; margin-bottom: 7px;">Bill Details:</h3>
            ...
            <td style="width: 50%; text-align: right; vertical-align: top; padding-left: 15px;">
              <h3 style="font-size: 12pt; font-weight: bold; margin-bottom: 7px;">${customerLabel}</h3>
              <p style="margin: 5px 0; font-weight: bold;">${bill.customerName || 'N/A'}</p>
              ${bill.customerAddress ? `<p style="font-size: 10pt; margin: 5px 0;">${bill.customerAddress}</p>` : ''}
              ${bill.customerPhone ? `<p style="font-size: 10pt; margin: 5px 0;">Phone: ${bill.customerPhone}</p>` : ''}
            </td>
          </tr>
        </table>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 25px; font-size: 10pt;">
          <thead>
            <tr style="font-size: 10pt;">${tableHeaders}</tr>
          </thead>
          <tbody>${itemsHtml}</tbody>
        </table>
        
        <hr style="border: 0; border-top: 1.5px solid #555555; margin: 25px 0;" />

        <table class="bill-summary-grid" style="width: 100%; margin-top: 25px; font-size: 11pt;">
          <tr>
            <td style="width: 60%; vertical-align: top; white-space: pre-line;">
              ${bill.notes ? `<h4 style="font-weight: bold; margin-bottom: 6px; font-size: 11pt;">Notes:</h4><p style="font-size:10pt;">${bill.notes}</p>` : ''}
            </td>
            <td style="width: 40%; text-align: right; vertical-align: top; padding-left: 20px;">
              ...
              ${isEstimateView && bill.type === 'sales-bill' ? `<p style="font-size: 9pt; color: #333; margin: 6px 0;">(GST not applicable for estimates)</p>` : ''}
              <hr style="border: 0; border-top: 1px solid #555555; margin: 10px 0;" />
              <p style="font-size: 14pt; font-weight: bold; margin-top: 10px;">Total: <span style="font-weight: bold; font-size: 16pt;">${currency}${bill.totalAmount.toFixed(2)}</span></p>
            </td>
          </tr>
        </table>

        <div class="bill-footer" style="text-align: center; margin-top: 35px; padding-top: 20px; border-top: 2px solid #000000; font-size: 10pt;">
  ...
  const handleGeneratePdf = async () => {
    ...
    // Save original bill content's parent and a placeholder
  ...
  const showHsnColumnInModal = bill.type === 'sales-bill' && !isEstimateView && bill.type !== 'purchase';
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-h-[95vh] flex flex-col print-dialog-content text-xl w-full max-w-screen-lg"> 
        ...
        <div className="flex-grow overflow-y-auto p-1" > 
            {/* This #bill-to-print is for the MODAL DISPLAY ONLY. PDF uses generatePdfHtml(). */}
